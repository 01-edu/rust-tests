...
fn initials(names: &mut Vec<&str>) -> Vec<String> {
    names.iter().map(|x| {
        ...
        let mut a = two_names.next().unwrap().chars().nth(0).unwrap().to_string();
        a.push_str(". ");
        a.push_str(&two_names.next().unwrap().chars().nth(0).unwrap().to_string());
        a.push_str(".");
        a
    }).collect()
}
...
mod tests {
    ...
    fn test_memory_allocation() {
        ...
        let mut test_value = vec!["Lee Silva", "Harry Potter", "Someone Else", "J. L.", "Barack Obama"];
    ...
    fn test_function() {
        let cases = vec![
            ...
            Test {
                names: vec!["James John", "David Joseph", "Matthew Brian", "Jacob Sousa", "Bruce Banner", "Scarlett Johansson", "Graydon Hoare"],
                result: vec!["J. J.", "D. J.", "M. B.", "J. S.", "B. B.", "S. J.", "G. H."]
            }
        ];

        for mut v in cases {
            assert_eq!(initials(&mut v.names), v.result.iter().map(|ele| ele.to_string()).collect::<Vec<String>>());
        }
    }
}
