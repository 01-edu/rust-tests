...
fn highest_paid_employee(mall: mall::Mall) -> Vec<employee::Employee> {
    let mut res = vec![employee::Employee::new("", 0, 0, 0, 0.0)];
    for elem in mall.floors.iter() {
        for shop in elem.stores.iter() {
            for mut emp in shop.employees.clone().into_iter() {
...
fn fire_old_securities(mall: &mut mall::Mall) {
...
fn check_for_securities(mall: &mut mall::Mall, available_sec: Vec<security::Security>) {
    let mut size = 0;
