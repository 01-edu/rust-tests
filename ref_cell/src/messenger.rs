    ...
    T: Logger,
{
    ...
    pub fn set_value(&self, value: &Rc<usize>) {
        ...
        if percentage_of_max >= 100 {
            ...
            self.logger
                .warning(&format!("Warning: you have used up over {}% of your quota! Proceeds with precaution", percentage_of_max));
        }
    }

    pub fn peek(&self, value: &Rc<usize>) {
        let percentage_of_max = convert_percentage(self.max, Rc::strong_count(&value));
        self.logger.info(&format!("Info: you are using up too {}% of your quote", percentage_of_max))
    }
}
