...
fn area_fit(
    ...
    let (max_size, mut size) = (x * y, 0.0);
    match objects {
        Geometrical_Shapes::Square => size = square_area(a) as f64,
        Geometrical_Shapes::Circle => size = circle_area(a),
        Geometrical_Shapes::Rectangle => size = rectangle_area(a, b) as f64,
        Geometrical_Shapes::Triangle => size = triangle_area(a, b),
    }
...
fn volume_fit(
    ...
    y: usize,
    z: usize,
    objects: Geometrical_Volumes,
    times: usize,
    a: usize,
    b: usize,
    c: usize,
) -> bool {
    let (max_size, mut size) = (x * y * z, 0.0);
    match objects {
        Geometrical_Volumes::Cube => size = cube_volume(a) as f64,
        Geometrical_Volumes::Sphere => size = sphere_volume(a),
        Geometrical_Volumes::Parallelepiped => size = parallelepiped_volume(a, b, c) as f64,
        Geometrical_Volumes::Pyramid => size = triangular_pyramid_volume(triangle_area(a, b), c),
        Geometrical_Volumes::Cone => size = cone_volume(a, b),
    }
    times as f64 * size <= max_size as f64
}
